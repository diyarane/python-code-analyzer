"""JavaScript & JSX parser implementation using Tree-sitter."""
...
class JavaScriptParser(BaseParser):
    """Parses JavaScript and JSX source code into a Tree-sitter AST."""

    def __init__(self, is_jsx: bool = False):
        self.is_jsx = is_jsx
        self._parser = get_parser("tsx" if is_jsx else "javascript")
    ...
    def parse(self, source_code: str) -> NormalizedSyntaxTree:
        ...
        if root.has_error:
            # Locate first syntax error node
            error_node = self._find_first_error(root)
            line = (error_node.start_point[0] + 1) if error_node else 1
            raise SyntaxError(f"JavaScript syntax error at line {line}", ("code", line, 1, source_code))

        lang_id = "javascript_jsx" if self.is_jsx else "javascript"
        return NormalizedSyntaxTree(raw_tree=tree, source_code=source_code, language_id=lang_id)
