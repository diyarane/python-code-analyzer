"""JavaScript & JSX analysis engine implementation."""
...
class JavaScriptAnalyzer(BaseAnalyzer):
    """Analyzes JavaScript / JSX Tree-sitter ASTs for complexity, AST JSON tree, and explanations."""
    ...
    def _build_explanations(self, metrics: dict, lang_id: str) -> dict:
        lang_title = "JavaScript/JSX" if "jsx" in lang_id else "JavaScript"
        ...
        return {
            "summary": (
                ...
                f"Analyzed control flow, statement depth, and syntax constructs."
            ),
...
def serialize_ts_node(node, source_code: str, depth: int = 0) -> dict:
    """Recursively convert Tree-sitter CST node into frontend AST node schema."""
    ...
    children = []
    # Filter out redundant punctuation tokens for clean visual AST graph
    for child in node.children:
