...
from .cpp_dead_code import CPPDeadCodeAnalyzer
...
class CPPAnalyzer(BaseAnalyzer):
    ...
    def analyze(
        ...
        ast_json = serialize_cpp_node(root_node, source_code)
    ...
    def _compute_metrics(self, root_node, tree: NormalizedSyntaxTree, source_code: str) -> Dict[str, Any]:
        ...
        dead_res = CPPDeadCodeAnalyzer().analyze(tree, source_code)
...
def build_cpp_label(node, source_code: str) -> str:
    ...
    if node.type in ("for_statement", "for_range_loop", "while_statement", "do_statement"):
