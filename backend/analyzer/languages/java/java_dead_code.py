"""
Java Tree-sitter Dead Code Analyzer implementation.
"""
...
class JavaDeadCodeAnalyzer(BaseDeadCodeAnalyzer):
    """Dead code analyzer for Java Tree-sitter CSTs."""
    ...
    def get_capability(self) -> DeadCodeCapability:
        return DeadCodeCapability(
            ...
            reason="Java analyzer inspects unreachable statements after return/throw/break/continue and static dead branches while preserving public APIs.",
        )
    ...
    def analyze(self, syntax_tree: Any, source_code: str) -> DeadCodeResult:
        ...
        return DeadCodeResult(
            ...
            reason=f"Found {len(findings)} dead code findings in Java source.",
        )

    def _traverse_node(self, node: Any, findings: List[DeadCodeFinding], source_code: str):
        # 1. Check unreachable statements in blocks or method bodies
        if node.type in ("block", "constructor_body"):
            self._check_unreachable_statements_in_block(node, findings)

        # 2. Check dead if (false) / if (0) branches
    ...
    def _is_statically_false(self, cond_node: Any, source_code: str) -> bool:
        ...
        if text.startswith("(") and text.endswith(")"):
            text = text[1:-1].strip()
        return text in ("false", "0", "!true", "null")
