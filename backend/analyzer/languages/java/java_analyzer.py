"""Java analysis engine implementation."""
...
from .java_dead_code import JavaDeadCodeAnalyzer


class JavaAnalyzer(BaseAnalyzer):
    """Analyzes Java Tree-sitter ASTs for complexity, AST JSON tree, and explanations."""
    ...
    def analyze(
        ...
        ast_json = serialize_java_node(root_node, source_code)
    ...
    def _compute_metrics(self, root_node, tree: NormalizedSyntaxTree, source_code: str) -> Dict[str, Any]:
        ...
        dead_res = JavaDeadCodeAnalyzer().analyze(tree, source_code)
    ...
    def _get_max_cond_depth(self, node, current_depth: int) -> int:
        is_cond = node.type in {"if_statement", "switch_expression", "switch_label"}
...
def serialize_java_node(node, source_code: str, depth: int = 0) -> dict:
    """Convert Java Tree-sitter node into frontend AST node schema."""
    start_line = node.start_point[0] + 1
    end_line = node.end_point[0] + 1

    node_type = format_node_type(node.type)
    label = build_java_label(node, source_code)
    weight = 2 if "statement" in node.type or "declaration" in node.type else 1

    children = []
    for child in node.children:
        if child.is_named:
            children.append(serialize_java_node(child, source_code, depth + 1))
...
def build_java_label(node, source_code: str) -> str:
