"""Go analysis engine implementation."""
...
class GoAnalyzer(BaseAnalyzer):
    """Analyzes Go Tree-sitter ASTs for complexity, AST JSON tree, and explanations."""
    ...
    def analyze(
        ...
        ast_json = serialize_go_node(root_node, source_code)
    ...
    def _get_max_cond_depth(self, node, current_depth: int) -> int:
        is_cond = node.type in {"if_statement", "expression_switch_statement", "type_switch_statement"}
...
def serialize_go_node(node, source_code: str, depth: int = 0) -> dict:
    """Convert Go Tree-sitter node into frontend AST node schema."""
    start_line = node.start_point[0] + 1
    end_line = node.end_point[0] + 1

    node_type = format_node_type(node.type)
    label = build_go_label(node, source_code)
    weight = 2 if "statement" in node.type or "declaration" in node.type else 1

    children = []
    for child in node.children:
        if child.is_named:
            children.append(serialize_go_node(child, source_code, depth + 1))
...
def build_go_label(node, source_code: str) -> str:
    ...
    if node.type in ("function_declaration", "method_declaration"):
        ...
        if name_node:
            ...
            return f"Func: {name_text}"
        return "Func"

    if node.type == "for_statement":
        return "Loop: ForStatement"
